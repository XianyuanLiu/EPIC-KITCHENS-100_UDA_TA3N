...
if args.noun_target_data is not None:
    data_set = TSNDataSet(args.test_target_data + ".pkl", args.test_list, num_dataload=num_test,
...
data_loader = torch.utils.data.DataLoader(data_set, batch_size=args.bS, shuffle=False, num_workers=args.workers,
                                          pin_memory=True)
...
def validate(val_loader, verb_model, criterion, num_class, noun_model=None, val_labels=False):
    ...
    with open(args.result_json, "w") as f:
        json.dump({'results_target': results_dict,
                   ...
                   "version": "0.2",
                   "challenge": "domain_adaptation", "sls_pt": 2,
                   "sls_tl": 3,
                   "sls_td": 3}, f)
    if val_labels:
